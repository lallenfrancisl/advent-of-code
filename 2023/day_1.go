...
func Day1(inp string) int {
	...
	for i, v := range items {
		items[i] = strings.Trim(v, " \t")
	}

	sum := 0

	for _, v := range items {
		value := getCalibrationValue(v, numMap)
		sum += value
	}
...
var validValue = regexp.MustCompile(
	`(oneight|twone|threeight|fiveight|sevenine|eightwo|eighthree|nineight|zerone|one|two|three|four|five|six|seven|eight|nine|\d)`,
)

func getCalibrationValue(text string, numMap map[string][]string) int {
	values := validValue.FindAll([]byte(text), -1)

	if values == nil {
		return 0
	}

	numLetters := convertToNumberLetters(values, numMap)

	if len(numLetters) < 1 {
		return 0
	}

	numStr := numLetters[0] + numLetters[len(numLetters)-1]
...
func convertToNumberLetters(items [][]byte, numMap map[string][]string) []string {
	numLetters := []string{}
	...
	for _, v := range items {
		...
		if err == nil {
			numLetters = append(numLetters, word)

			continue
		}

		numLetter, ok := numMap[string(v)]

		if ok {
			numLetters = append(numLetters, numLetter...)
		}
	}
